...
interface FastElaDataViewProps {
  studentId: number;
}
...
export default function FastElaDataView({ studentId }: FastElaDataViewProps) {
  // Detect dark mode
  ...
  const { data: assessmentData, isLoading } = useQuery({
    queryKey: ['fast-ela-detailed', studentId],
    queryFn: async () => {
      // Fetch FAST ELA assessments using compound ID utility
      const allAssessments = await queryByStudentId(db.assessments, studentId);
      const assessments = allAssessments.filter(record => 
        (record.source?.includes('FAST') && record.subject === 'ELA') ||
        (record.source === 'FAST' && (record.subject === 'Reading' || record.subject === 'ELA'))
      );
      
      console.log(`📊 Found ${assessments.length} FAST ELA assessments for student ${studentId}`);
      ...
      assessments.forEach(assessment => {
        ...
        console.log(`📊 ELA Assessment - Period: ${period}, Score: ${assessment.score}, Date: ${assessment.testDate}, PercentileRank: ${assessment.percentile}`);
      ...
      console.log('📊 ELA Grouped by period:', groupedByPeriod);
      console.log('📊 ELA Sample assessment details:', assessments[0]);
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-gray-600 dark:text-gray-400">Loading FAST ELA data...</div>
      </div>
    );
  }
  ...
  if (!assessmentData || Object.keys(assessmentData).length === 0) {
    return (
      <div className="text-center py-8 text-gray-600 dark:text-gray-400">
        No FAST ELA assessment data available for this student.
      </div>
    );
  }
  ...
  console.log('📊 Available periods in ELA:', availablePeriods);
  console.log('📊 All ELA assessment data keys:', assessmentData ? Object.keys(assessmentData) : 'No data');
  ...
  // ELA-specific performance categories
  const elaCategories = [
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">
            FAST ELA Reading Assessment Results
          </h3>
        </div>
        ...
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Line 
              ...
              strokeWidth={3}
              dot={{ fill: isDarkMode ? '#a855f7' : '#9333ea', strokeWidth: 2, r: 6 }}
              connectNulls={false}
            />
          </LineChart>
        </ResponsiveContainer>
      ...
      <div className="text-center">
        <button
          ...
          className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 dark:bg-purple-500 dark:hover:bg-purple-600"
        >
              ...
              <div>
                <div className="mt-3 text-center sm:mt-0 sm:text-left">
                  <h3 className="text-xl leading-6 font-bold text-gray-900 dark:text-gray-100 mb-6" id="modal-title">
                    Complete FAST ELA Reading Assessment Report
                  </h3>
                      ...
                      {availablePeriods.map(period => {
                        ...
                        return (
                          <div key={period} className="border border-gray-200 dark:border-gray-600 rounded-lg">
                            {/* Header */}
                            <div className="bg-purple-50 dark:bg-purple-900/20 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
