...
export interface IReadyReadingUploadResult {
  totalRows: number;
  matchedRows: number;
...
export class IReadyReadingAdapter {
  async processFile(file: File): Promise<IReadyReadingUploadResult> {
    console.log('📖 Starting iReady Reading file processing...');
    
    const csvText = await this.readFileAsText(file);
    const parsedData = await this.parseCSV(csvText);
    
    console.log(`📊 Parsed ${parsedData.length} rows from iReady Reading CSV`);
    ...
    console.log(`📝 Processing ${dataRows.length} iReady Reading assessment records`);
    
    const result: IReadyReadingUploadResult = {
    ...
    // Clear existing iReady Reading assessments
    await db.assessments.where('source').equals('iReady Reading').delete();
    console.log('🗑️  Cleared existing iReady Reading assessment data');
      ...
      try {
        ...
        if (match) {
          ...
          if (assessmentRecord) {
            ...
            console.log(`  📖 Added iReady Reading assessment: Overall Score ${assessmentRecord.score}, Placement ${assessmentRecord.proficiency}`);
          }
    ...
    console.log('📊 iReady Reading processing complete:', result);
    return result;
  }
  ...
  private createAssessmentRecord(
    ...
    try {
      ...
      // Parse domain scores
      const phonologicalAwarenessScore = parseInt(rowData['Phonological Awareness Scale Score'] || '0', 10);
      const phonicsScore = parseInt(rowData['Phonics Scale Score'] || '0', 10);
      const highFrequencyWordsScore = parseInt(rowData['High-Frequency Words Scale Score'] || '0', 10);
      const vocabularyScore = parseInt(rowData['Vocabulary Scale Score'] || '0', 10);
      const comprehensionOverallScore = parseInt(rowData['Comprehension: Overall Scale Score'] || '0', 10);
      const comprehensionLiteratureScore = parseInt(rowData['Comprehension: Literature Scale Score'] || '0', 10);
      const comprehensionInformationalScore = parseInt(rowData['Comprehension: Informational Text Scale Score'] || '0', 10);
      ...
      const assessmentRecord: AssessmentRecord = {
        ...
        // Store all iReady Reading-specific data
        originalStudentName,
        ...
        lexileMeasure: rowData['Lexile Measure'] || '',
        lexileRange: rowData['Lexile Range'] || '',
        // Domain scores - Reading-specific domains
        phonologicalAwarenessScore: phonologicalAwarenessScore || undefined,
        phonologicalAwarenessPlacement: rowData['Phonological Awareness Placement'] || '',
        phonologicalAwarenessRelativePlacement: rowData['Phonological Awareness Relative Placement'] || '',
        phonicsScore: phonicsScore || undefined,
        phonicsPlacement: rowData['Phonics Placement'] || '',
        phonicsRelativePlacement: rowData['Phonics Relative Placement'] || '',
        highFrequencyWordsScore: highFrequencyWordsScore || undefined,
        highFrequencyWordsPlacement: rowData['High-Frequency Words Placement'] || '',
        highFrequencyWordsRelativePlacement: rowData['High-Frequency Words Relative Placement'] || '',
        vocabularyScore: vocabularyScore || undefined,
        vocabularyPlacement: rowData['Vocabulary Placement'] || '',
        vocabularyRelativePlacement: rowData['Vocabulary Relative Placement'] || '',
        comprehensionOverallScore: comprehensionOverallScore || undefined,
        comprehensionOverallPlacement: rowData['Comprehension: Overall Placement'] || '',
        comprehensionOverallRelativePlacement: rowData['Comprehension: Overall Relative Placement'] || '',
        comprehensionLiteratureScore: comprehensionLiteratureScore || undefined,
        comprehensionLiteraturePlacement: rowData['Comprehension: Literature Placement'] || '',
        comprehensionLiteratureRelativePlacement: rowData['Comprehension: Literature Relative Placement'] || '',
        comprehensionInformationalScore: comprehensionInformationalScore || undefined,
        comprehensionInformationalPlacement: rowData['Comprehension: Informational Text Placement'] || '',
        comprehensionInformationalRelativePlacement: rowData['Comprehension: Informational Text Relative Placement'] || '',
        // Growth metrics
        ...
        readingDifficultyIndicator: rowData['Reading Difficulty Indicator (Y/N)'] || ''
      };
      
      return assessmentRecord;
      
    } catch (error) {
      console.error('Error creating iReady Reading assessment record:', error);
      return null;
    }
  }
  ...
  private mapPlacementToProficiency(placement: string, relativePlacement: string): 'below' | 'approaching' | 'meets' | 'exceeds' {
    ...
    if (relativePlacementLower.includes('on') || relativePlacementLower.includes('at grade level') || relativePlacementLower.includes('mid or above grade level')) {
      return 'meets';
    }
...
export const iReadyReadingAdapter = new IReadyReadingAdapter();