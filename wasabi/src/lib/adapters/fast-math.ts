...
export class FastMathAdapter implements DataAdapter {
  name = 'FAST Math Assessment (Grades 3-5)';
  type = 'fast-math' as const;
  ...
  validateData(data: ParsedData): ValidationResult {
    ...
    if (!hasMathIndicators && !hasTestSubject) {
      warnings.push('No math subject indicators found - verify this is FAST Math data');
    }
  ...
  async transformDataWithMatching(data: ParsedData): Promise<{
    ...
    for (const matchedRow of matchedRows) {
      try {
        ...
        console.warn('Error transforming FAST Math record:', error);
      }
    }
  ...
  private transformFastRecord(row: Record<string, any>, headers: string[]): AssessmentRecord | null {
    ...
    const achievementLevel = this.findValue(row, headers, [
      ...
      'FAST Mathematics Achievement Level'
    ]);
    ...
    // Determine subject - should be Math for this adapter
    let subject = 'Math';
    if (testSubject) {
      const subjectStr = String(testSubject).toLowerCase();
      if (subjectStr.includes('math') || subjectStr.includes('mathematics')) {
        subject = 'Math';
      }
    }
