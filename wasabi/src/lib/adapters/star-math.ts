...
export class StarMathAdapter implements DataAdapter {
  name = 'STAR Math Assessment (Grades K-2)';
  type = 'star-math' as const;
  ...
  validateData(data: ParsedData): ValidationResult {
    ...
    const hasMathIndicators = headers.some(h => 
      h.includes('math') || h.includes('mathematics') ||
      h.includes('star') && h.includes('math')
    );
    ...
    if (!hasMathIndicators) {
      warnings.push('No math indicators found - verify this is STAR Math data');
    }
  ...
  async transformData(data: ParsedData): Promise<AssessmentRecord[]> {
    ...
    for (const row of data.rows) {
      try {
        const record = this.transformStarRecord(row, data.headers);
        if (record) {
          records.push(record);
        }
      } catch (error) {
        console.warn('Error transforming STAR Math record:', error);
      }
    }
  ...
  private transformStarRecord(row: Record<string, any>, headers: string[]): AssessmentRecord | null {
    ...
    // Filter for Math tests only
    const activityName = this.findValue(row, headers, [
      'Activity Name', 'activity_name', 'test_name', 'test_type'
    ]);
    
    if (activityName && !String(activityName).toLowerCase().includes('math')) {
      return null; // Skip non-Math tests
    }

    if (!studentId || !scaledScore || !testDate) {
      return null;
    }
    ...
    if (testName) {
      ...
      if (testStr.includes('math') || testStr.includes('mathematics')) {
        subject = 'Math';
      }
    }
